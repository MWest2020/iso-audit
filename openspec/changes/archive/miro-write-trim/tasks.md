...
- [x] 1.1 Quality-gate volledig groen op `refactor/iso-audit-milestone-b`
  ...
  bij uitvoering.
- [x] 1.2 Functionele baseline: `uv run iso-audit doctor` — verifieer
...
- [x] 1.3 Pipeline-baseline: `uv run iso-audit pipeline --source drive
...
- [x] 2.1 Verwijder `src/iso_audit/miro/interview.py` +
...
- [x] 2.2 Verwijder `src/iso_audit/miro/board_setup.py` +
...
- [x] 2.3 Verifieer `src/iso_audit/miro/__init__.py` ongewijzigd
  (exports `MiroClient`, `MiroError`, `MiroRateLimitError`). Geen
  imports van weggehaalde modules in `__all__`.

- [x] 2.4 Coverage-check: `uv run pytest --cov=iso_audit --cov-report=term
...
- [x] 3.1 Verhuis `openspec/changes/miro-kennissessie-generator/` naar
  ...
  naar deze change.
- [x] 3.2 Update `openspec/changes/archive/iso-refactor/tasks.md` indien
  nodig (referenties naar `board_setup` / `interview` weghalen).

## 4. Documentatie — alternatief voor write-pad

- [x] 4.1 Schrijf `docs/miro-auditor-bord-prompt.md`:
...
- [x] 4.2 Schrijf `docs/miro-interview-prompt.md` als equivalent voor de
...
- [x] 5.1 Pre-merge dry-run: `uv run iso-audit pipeline --source drive
...
- [x] 5.2 Volledige test-suite groen (649 passed, ~1 skipped).
- [x] 5.3 CI-pipeline groen op de PR (lint + format + typecheck +
  security + test + coverage ≥ 70%).
- [x] 5.4 Coverage ≥ 80% overall (was 81%; verwacht +1–2pp omdat de
  modules met laagste coverage weg zijn).
- [x] 5.5 README/CLAUDE.md: noem `docs/miro-auditor-bord-prompt.md` en
...
- [ ] 5.6 Tag `v0.3.0-beta` (of vergelijkbaar) — eerste post-M-C trim.
  _Overgeslagen: MEMORY.md mikt op één gebundelde `v1.0.0` na M-C §3.6
  acceptatie, geen tussentijdse tag._
...
- [x] 6.1 De code blijft in de git-historie beschikbaar. Een revert
  van de twee delete-commits zet alles terug. Het bestand
  `docs/miro-auditor-bord-prompt.md` wordt dan een aanvulling i.p.v.
  vervanging.
- [x] 6.2 Mocht een specifieke functie (bv. `_grid_position` in
